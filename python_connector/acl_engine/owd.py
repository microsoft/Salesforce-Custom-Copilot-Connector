...
# schema.json lives at: python_connector/connector/references/schema.json
# acl_engine is at:      python_connector/acl_engine/
# So two levels up, then into connector/references/
_DEFAULT_SCHEMA_PATH: Path = (
    Path(__file__).resolve().parent.parent / "connector" / "references" / "schema.json"
)
...
def _load_owd_field_map(schema_path: Path) -> dict[str, str]:
    """
    Read schema.json and return ``{objectName: owdField}`` for every object
    that declares an ``owdField``.
    ...
    Returns an empty dict on any I/O or parse error so the caller defaults
    every object to "Private" safely.
    """
    try:
        with schema_path.open("r", encoding="utf-8") as fh:
            config: dict = json.load(fh)
    except (OSError, json.JSONDecodeError) as exc:
        logger.warning("[OWD] Cannot load schema.json (%s): %s", schema_path, exc)
        return {}

    mapping = {
        obj["objectName"]: obj["owdField"]
        for obj in config.get("objectList", [])
        if obj.get("objectName") and obj.get("owdField")
    }
...
class OWDFetcher:
    ...
    schema_path : Override path to schema.json.  Defaults to the bundled
    ...
    def __init__(
        ...
        self._owd_field_map: dict[str, str] = _load_owd_field_map(
