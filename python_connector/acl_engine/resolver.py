...
def _load_parent_map(schema_path: Path) -> dict[str, tuple[str, str]]:
    """
    Read schema.json and return {objectName: (parentFieldName, parentObjectName)}
    ...
    for obj in data.get("objectList", []):
        ...
        if obj_name and parent_obj:
            # Explicit key wins; otherwise derive standard {ParentName}Id field
...
class AclResolver:
    ...
    def __init__(self, sf_client: SalesforceClient, schema_path: Path = _DEFAULT_SCHEMA_PATH) -> None:
        self._sf = sf_client
        self._owd_fetcher = OWDFetcher(sf_client)
        ...
        self._parent_map: dict[str, tuple[str, str]] = _load_parent_map(schema_path)
