...
export async function ingestContent(config: Config, since?: Date): Promise<void> {
  config.context.log('🚀 Starting ingestion process...');
  if (since) {
    config.context.log(`📅 Incremental sync from: ${since.toISOString()}`);
  } else {
    config.context.log('📅 Full sync (all items)');
  }
  ...
  for await (const item of getAllItems(config, since)) {
    ...
    if (ingestedCount % 10 === 0) {
      config.context.log(`   ⏳ Ingested ${ingestedCount} items so far...`);
    }
  ...
  // Final summary
  config.context.log('');
  config.context.log('✅ Ingestion complete!');
  config.context.log(` Total items ingested: ${ingestedCount}`);
}
